  ...
  return (
    <footer className="platform-footer" id="about">
      <div className="footer-main">
        ...
        <div><b>{isArabic ? "المنصة" : "Platform"}</b><Link to="/portfolios">{isArabic ? "المحافظ" : "Portfolios"}</Link><Link to="/recommendations">{isArabic ? "التوصيات" : "Recommendations"}</Link><Link to="/weekly-reports">{isArabic ? "التقارير الأسبوعية" : "Weekly reports"}</Link><Link to="/news">{isArabic ? "الأخبار والتحليل" : "News & Analysis"}</Link></div>
        <div><b>{isArabic ? "المعرفة" : "Intelligence"}</b><Link to="/methodology">{isArabic ? "المنهجية" : "Methodology"}</Link><Link to="/recommendations">{isArabic ? "الأبحاث" : "Research"}</Link><a href="/compound-interest-calculator">{isArabic ? "حاسبة الفائدة المركبة" : "Compound Interest Calculator"}</a><a href="/#pricing">{isArabic ? "الأسعار" : "Pricing"}</a></div>
