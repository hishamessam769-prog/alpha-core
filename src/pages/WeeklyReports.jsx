...
export default function WeeklyReports() {
  ...
  useEffect(() => {
    const load = async () => {
      const { data, error } = await supabase.from("weekly_reports").select("*").eq("is_published", true).order("week_end", { ascending: false });
      ...
      <main className="weekly-page-v23 weekly-page-v3">
        ...
        {loading ? <div className="loading-screen"><div className="loader-ring"/></div> : (
          <section className="weekly-timeline-v3">
            {visible.map((report, index) => <article key={report.id}><div className="weekly-timeline-rail-v3"><span/><b>{getWeekNumber(report.week_end)}</b><i/></div><Link className="weekly-card-v23 weekly-card-v3" to={`/weekly-reports/${report.slug}`}><header><span className="eyebrow">WEEK {getWeekNumber(report.week_end)}</span><em>{index === 0 ? (isArabic ? "الأحدث" : "LATEST") : "REPORT"}</em></header><h2>{report.title}</h2><p>{report.summary}</p><div className="weekly-card-sections-v3"><span><FileText size={14}/> Market recap</span><span><TrendingUp size={14}/> Portfolio update</span><span><Sparkles size={14}/> Outlook</span></div><footer><span><CalendarRange size={14}/>{new Date(`${report.week_start}T12:00:00`).toLocaleDateString(locale)} — {new Date(`${report.week_end}T12:00:00`).toLocaleDateString(locale)}</span><b>{isArabic ? "فتح التقرير" : "Open report"}<ArrowUpRight size={15}/></b></footer></Link></article>)}
