...
import { workspaceRoute } from "../lib/access";
...
export default function Landing() {
  ...
  if (session) return <Navigate to={profile?.is_admin ? workspaceRoute(profile) : "/dashboard"} replace />;
    ...
    <div className="public-page landing-v3">
      ...
      <main>
        <section className="hero-v3">
          ...
          <Reveal className="terminal-showcase terminal-v3" delay={90} aria-label="ALPHA PLATFORM product preview">
        ...
        <Reveal as="section" className="landing-highlight-strip-v31" delay={80} aria-label={isArabic ? "أبرز نتائج المنصة" : "Platform performance highlights"}>{highlights.slice(0, 3).map((item, index) => <article className="landing-highlight-v31" key={`${item.label_en}-${index}`}><span>0{index + 1}</span><div><b>{item.value}</b><h3>{isArabic ? item.label_ar : item.label_en}</h3><p>{isArabic ? item.detail_ar : item.detail_en}</p></div></article>)}</Reveal>
        ...
        <Reveal as="section" className="public-news-preview-v32" delay={100}>
          <div className="public-news-copy-v32"><span className="eyebrow">MARKET NEWS & ECONOMIC EVENTS</span><h2>{isArabic ? "صورة السوق قبل أن تدخل في التفاصيل" : "See the market picture before you enter the detail"}</h2><p>{isArabic ? "بعد تسجيل الدخول، يجمع موجز السوق آخر تقرير وتحديثات الشركات والأحداث التي نراقبها بجوار أداء المحفظة والمؤشر." : "Once signed in, the market intelligence widget places the latest report, company updates and events to watch beside portfolio and benchmark performance."}</p><div className="public-news-pills-v32"><span><Newspaper/>{isArabic ? "تقارير أسبوعية" : "Weekly reports"}</span><span><TrendingUp/>{isArabic ? "تحديثات الشركات" : "Company updates"}</span><span><FileClock/>{isArabic ? "أحداث قادمة" : "Upcoming events"}</span></div><Link className="button subtle" to="/signup">{isArabic ? "استكشف موجز السوق" : "Explore market intelligence"}<ArrowRight size={15}/></Link></div>
          <MarketGraphic label="NEWS · EVENTS · BENCHMARKS"/>
        </Reveal>

        <Reveal as="section" className="product-proof-section product-proof-v3" delay={80}><div className="section-heading centered"><span className="eyebrow">INSTITUTIONAL PERFORMANCE VIEW</span><h2>{isArabic ? "كل رقم في مكانه. كل قرار له تاريخ." : "Every number in context. Every decision on record."}</h2><p>{isArabic ? "لوحة أداء واحدة تربط عائد المحفظة بالمؤشر والألفا والتوزيع والقرارات الشهرية." : "One performance view connects portfolio return, benchmark, Alpha, allocation and every monthly decision."}</p></div><div className="six-metric-preview"><PreviewKpi label={t("portfolioMtd")} value={previewPortfolio} tone="blue"/><PreviewKpi label={t("benchmarkMtd")} value={previewBenchmark} tone="gold"/><PreviewKpi label={t("monthlyAlpha")} value={previewAlpha} tone="green"/><PreviewKpi label={t("cumulativePortfolio")} value="+13.40%" tone="blue"/><PreviewKpi label={t("cumulativeBenchmark")} value="+8.20%" tone="gold"/><PreviewKpi label={t("cumulativeAlpha")} value="+5.20%" tone="green"/></div></Reveal>

        <Reveal as="section" className="intelligence-section-v3" delay={100}><div className="section-heading"><span className="eyebrow">LATEST INTELLIGENCE</span><h2>{isArabic ? "الأبحاث والتقارير في تجربة تشبه المجلة الاستثمارية" : "Research and reporting built like a premium investment journal"}</h2><p>{isArabic ? "اقرأ الفرضية والمخاطر والتقييم والتحديثات من غير تشتيت أو مبالغة." : "Read the thesis, risks, valuation and updates without noise or promotional clutter."}</p></div><div className="intelligence-grid-v3">{intelligenceCards.map(({ icon: Icon, tag, title, text, meta }, index) => <article className={index === 0 ? "featured" : ""} key={tag}><div className="magazine-visual"><Icon/><span>0{index + 1}</span></div><span className="eyebrow">{tag}</span><h3>{isArabic ? ["تقرير المحفظة الشهري", "التوصيات المستقلة", "تقرير السوق الأسبوعي"][index] : title}</h3><p>{isArabic ? ["التوزيع الحالي والأداء مقابل المؤشر وسجل القرارات الكامل.", "سعر بداية ومستهدف وفرضية ومخاطر وتاريخ تحديثات دائم.", "مراجعة مؤسسية مختصرة للسوق والمحافظ والذهب والأسبوع القادم."][index] : text}</p><footer><small>{meta}</small><ArrowRight size={16}/></footer></article>)}</div></Reveal>

        <Reveal as="section" className="why-section-v21 why-v3" delay={100} id="about"><div className="section-heading"><span className="eyebrow">BUILT FOR TRUST</span><h2>{isArabic ? "الاحتراف يبدأ من الشفافية" : "Professional investing starts with transparent evidence"}</h2><p>{isArabic ? "المنصة مصممة لتقليل الضوضاء وإظهار ما يحتاجه المستثمر لاتخاذ قرار واعٍ." : "The platform removes noise and surfaces the evidence an investor needs to make an informed decision."}</p></div><div className="why-grid-v21"><Feature icon={<Target/>} number="01" title={isArabic ? "قرارات قابلة للقياس" : "Measurable decisions"} text={isArabic ? "كل قرار مرتبط بسعر وتاريخ ومؤشر مقارنة." : "Every decision is tied to a price, date and benchmark."}/><Feature icon={<BarChart3/>} number="02" title={isArabic ? "أداء مؤسسي" : "Institutional performance"} text={isArabic ? "عائد شهري وتراكمي وألفا واضحة." : "Monthly, cumulative and Alpha performance in context."}/><Feature icon={<FileClock/>} number="03" title={isArabic ? "سجل دائم" : "Permanent record"} text={isArabic ? "التغييرات والتحديثات محفوظة زمنيًا." : "Changes and updates remain permanently timestamped."}/><Feature icon={<Sparkles/>} number="04" title={isArabic ? "ملخصات ذكية" : "Intelligent summaries"} text={isArabic ? "شرح واضح للمحفظة والتوصيات والتقارير." : "Clear explanations of portfolios, recommendations and reports."}/><Feature icon={<UsersRound/>} number="05" title={isArabic ? "صلاحيات منظمة" : "Governed access"} text={isArabic ? "تجربة منفصلة للعضو والإدارة وSuper Admin." : "Purpose-built member, admin and Super Admin experiences."}/><Feature icon={<Globe2/>} number="06" title={isArabic ? "متجاوب بالكامل" : "Responsive everywhere"} text={isArabic ? "تجربة سريعة على الكمبيوتر والموبايل والتابلت." : "A fast, consistent experience across desktop, tablet and mobile."}/></div></Reveal>

        <Reveal as="section" className="pricing-section-v3" delay={100} id="pricing"><div className="section-heading centered"><span className="eyebrow">{isArabic ? settings.pricing_eyebrow_ar : settings.pricing_eyebrow_en}</span><h2>{isArabic ? settings.pricing_title_ar : settings.pricing_title_en}</h2><p>{isArabic ? settings.pricing_description_ar : settings.pricing_description_en}</p></div><article className="pricing-card-v3"><div><span className="eyebrow">ALPHA MEMBER</span><h3>{isArabic ? settings.pricing_plan_name_ar : settings.pricing_plan_name_en}</h3><p>{isArabic ? settings.pricing_plan_description_ar : settings.pricing_plan_description_en}</p></div><div className="price-v3"><b>{settings.pricing_price}</b><span>{isArabic ? settings.pricing_period_ar : settings.pricing_period_en}</span></div><ul>{priceFeatures.map((feature) => <li key={feature}><Check/>{feature}</li>)}</ul><Link className="button gold large" to="/signup">{isArabic ? settings.pricing_cta_ar : settings.pricing_cta_en}<ArrowRight size={16}/></Link></article></Reveal>

        <Reveal as="section" className="cta-v21 cta-v3" delay={100}><div><ShieldCheck size={31}/><span className="eyebrow">ALPHA PLATFORM V3.3</span><h2>{isArabic ? "ادخل منصة استثمارية مصممة للقرار" : "Enter an investment platform designed for decisions"}</h2><p>{isArabic ? "محافظ وأبحاث وتوصيات وتقارير في تجربة مؤسسية واحدة." : "Portfolios, research, recommendations and reports in one institutional experience."}</p></div><Link className="button gold large" to="/signup">{isArabic ? "ابدأ الآن" : "Get started"}<span>{arrow}</span></Link></Reveal>
      </main>
      <PlatformFooter />
    </div>
  );
